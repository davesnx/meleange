...
let read_folders_from_build ~target root =
  ...
  | `Directory -> Eio.Path.read_dir target_folder
  | `Not_found -> (* TODO: Fail hard *) []
...
let read_params_or_help () =
  if Array.length Sys.argv < 3 then
    failwith "Usage: importmap/generate.exe <target> <output_path>"
  else
...
let () =
  (* TODO: Add --verbose flag and print traceln or similars when turned on *)
